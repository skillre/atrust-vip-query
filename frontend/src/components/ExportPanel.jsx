...
export default function ExportPanel() {
  ...
  async function previewExport() {
    ...
    try {
      let url = `${API_BASE}/vip/history?name=${encodeURIComponent(name || '*')}&days=${days}&page=1&page_size=50`
      ...
      <div className="card">
        ...
        <div className="form-row">
          <div className="form-group">
            <label className="form-label" htmlFor="export-name">用户名筛选（可选）</label>
            <input
              type="text"
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="export-days">导出天数</label>
            <input
              type="number"
              className="form-input"
              id="export-days"
          ...
          <div className="form-group">
            <label className="form-label" htmlFor="export-type">事件类型</label>
            <select
              className="form-input"
              id="export-type"
              value={type}
        ...
        <div className="flex gap-3 mt-4">
          <button className="btn btn-secondary" onClick={previewExport} disabled={loading}>
            {loading ? '预览中...' : '预览数据'}
          </button>
