	...
	async function performReverseQuery(ipValue, limitValue) {
		...
		try {
			const resp = await fetch(
				`${API_BASE}/vip/reverse?ip=${encodeURIComponent(ipValue)}&limit=${limitValue}`,
			);
			...
			<div className="card">
				...
				<div className="form-row">
					<div className="form-group" style={{ flex: 2 }}>
						<label className="form-label" htmlFor="reverse-ip">
							虚拟IP地址
						</label>
						<input
							type="text"
							...
							onKeyDown={(e) =>
								e.key === "Enter" && performReverseQuery(ip, limit)
							}
						/>
					</div>
					<div className="form-group" style={{ flex: 1 }}>
						<label className="form-label" htmlFor="reverse-limit">
							返回条数
						</label>
						<input
							type="number"
					...
					{!loading && records && (
						<>
							<div className="flex items-center justify-between mb-4">
								...
								>
									虚拟IP: {virtualIp} · 共 {records.length} 条记录
								</span>
							</div>
							<table className="data-table">
								...
								<tbody>
									{records.map((r, i) => (
										<tr key={i}>
											<td className="mono">{r.user_name}</td>
											<td>{r.display_name || "-"}</td>
											<td className="mono">{r.real_ip || "-"}</td>
											<td>
												<span className="badge badge-info">{r.event_type}</span>
											</td>
